...
## 5. 출력 렌더링·품질 개선 (2026-06-03, v0.3.0)

사용자 피드백("번역 블록이 작고 폰트 불분명·들여쓰기/줄바꿈 안 됨", "긴 답변·md 아티팩트 garbage", "접기/펼치기 안 됨", "표 렌더 안 됨")을 실제 claude.ai에서 재현·수정:

| 문제 | 원인 | 수정 | 검증 |
|---|---|---|---|
| 긴 답변·마크다운 garbage("0000…", 줄바꿈 붕괴) | 온디바이스 NMT에 구조적 블록을 통째로 전달 | `translateMasked`를 **줄단위 번역**으로(코드펜스 분리, 비번역 줄 스킵) | 실측: 통짜=garbage, 줄단위=clean ✓ |
| 들여쓰기/줄바꿈/구조 안 보임 | 번역 블록을 원시 텍스트로 표시 | `lib/md-render.js` — 마크다운→DOM 렌더(헤딩/목록/코드블록/인용/표/볼드/링크) | 실제 답변에서 h3/h4/ol/pre/table 렌더 ✓ |
| 폰트 작고 불분명 | 0.95em + 상속 폰트 | 15.5px·line-height 1.78·한국어 폰트 스택 | ✓ |
| 표 렌더 안 됨 | 렌더러에 표 미지원 | `|…|` 표 → `<table>` 렌더 + CSS | 3×3 표 렌더 ✓ |
| 접기/펼치기 안 됨 | `.collapsed` 클래스 CSS가 claude.ai 스타일에 안 이김 | 인라인 `display:none !important` + stopPropagation | display none↔block 토글 ✓ |
| 긴 답변 스트리밍 중 조기 번역 | 완료 감지가 stop 버튼에만 의존 | 텍스트 길이 안정성(900ms) 가드 추가 | ✓ |

실제 claude.ai에서 코드/목록/표/아티팩트 답변을 생성→번역→렌더→토글까지 E2E 확인(온디바이스 줄단위 품질은 신선 프로필, 실제 추출·렌더·주입·토글은 사용자 세션에서). 데모: `assets/demo.png`.
