  ...
  async function translateMasked(text, translateFn) {
    ...
    for (const part of parts) {
      ...
      for (const line of lines) {
        ...
        // Peel off a leading markdown block marker (#, -, *, +, 1., >) and
        // translate ONLY the text after it, then re-prepend the exact marker.
        // Guarantees headings / lists / quotes keep their structure even if the
        // MT engine would otherwise drop or alter the marker.
        const mk = line.match(/^(\s*(?:#{1,6}\s+|[-*+]\s+|\d+[.)]\s+|>\s?))/);
        const prefix = mk ? mk[1] : "";
        const body = mk ? line.slice(prefix.length) : line;
        if (!isTranslatable(body)) { outLines.push(line); continue; }
        const { masked, store } = maskInline(body);
        let translated;
        try { translated = await translateFn(masked); }
        catch (e) { translated = masked; } // keep original on failure, don't drop the line
        outLines.push(prefix + unmaskInline(translated, store));
      }
